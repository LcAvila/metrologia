...
-- ETAPA 2: Criar tabela de usuários (mais importante para resolver o erro 500)
...
CREATE POLICY "Usuarios visualizam proprio perfil" ON usuarios
  FOR SELECT USING (auth.uid() = id);
  
CREATE POLICY "Admins gerenciam todos usuarios" ON usuarios
  USING (
    EXISTS (
...
CREATE POLICY "Todos visualizam equipamentos" ON equipamentos
  FOR SELECT USING (auth.role() = 'authenticated');
  
CREATE POLICY "Admin e metrologistas modificam equipamentos" ON equipamentos
  USING (
    EXISTS (
...
CREATE POLICY "Todos visualizam certificados" ON certificados
  FOR SELECT USING (auth.role() = 'authenticated');
  
CREATE POLICY "Admin e metrologistas modificam certificados" ON certificados
  USING (
    EXISTS (
      SELECT 1 FROM usuarios 
      WHERE id = auth.uid() 
...
CREATE POLICY "Todos visualizam FISPQs" ON fispqs
  FOR SELECT USING (auth.role() = 'authenticated');
  
CREATE POLICY "Admin e quimicos modificam FISPQs" ON fispqs
  USING (
    EXISTS (
      SELECT 1 FROM usuarios 
      WHERE id = auth.uid() 
...
CREATE POLICY "Todos visualizam Fichas de Emergencia" ON fichas_emergencia
  FOR SELECT USING (auth.role() = 'authenticated');
  
CREATE POLICY "Admin e quimicos modificam Fichas de Emergencia" ON fichas_emergencia
  USING (
    EXISTS (
      SELECT 1 FROM usuarios 
      WHERE id = auth.uid() 
